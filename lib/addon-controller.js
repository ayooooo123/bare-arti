...
const { matchingReachableAddresses, normalizeReachableAddresses } = require('./reachable-addresses')

const DEFAULT_TIMEOUT = 600000
const MIN_TIMEOUT = 1000
const MAX_TIMEOUT = 1800000
const ADDON_ABI_VERSION = 2
const ADDON_CAPABILITIES = 'reachableAddresses'
const DOCUMENTED_ERRORS = new Set([
  'ERR_ARTI_UNSUPPORTED_PLATFORM',
  'ERR_ARTI_ADDON_MISSING',
  'ERR_ARTI_ADDON_INCOMPATIBLE',
...
function validateAddonBinding(binding) {
  let snapshot
  try {
  ...
  if (
    !snapshot ||
    snapshot.abiVersion !== ADDON_ABI_VERSION ||
    snapshot.capabilities !== ADDON_CAPABILITIES ||
    typeof snapshot.start !== 'function' ||
    typeof snapshot.stop !== 'function'
  ) {
    throw artiError(
      'ERR_ARTI_ADDON_INCOMPATIBLE',
      `bare-arti addon ABI ${ADDON_ABI_VERSION} with ${ADDON_CAPABILITIES} is required`
    )
  }
...
function validateAddonOptions(options, dependencies) {
  ...
  const reachableAddresses = normalizeReachableAddresses(options.reachableAddresses)
  ...
  return Object.freeze({
    ...
    dataDir,
    reachableAddresses,
    reachableAddressesString: reachableAddresses === undefined ? '' : reachableAddresses.join(','),
    timeout
  })
}
