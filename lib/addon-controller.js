...
function nativeError(error) {
  ...
  return artiError('ERR_ARTI_BOOTSTRAP', 'Arti addon operation failed', error)
}
...
function createAddonController({ binding, validateOptions, setTimer, clearTimer }) {
  ...
  function beginStop(stopGeneration, startError) {
    ...
    stopping = Promise.resolve(nativeStopping).then(
      ...
      (error) => finishStop(stopGeneration, startError, wasStarting, nativeError(error))
    )
    return stopping
  }
  ...
  function start(options) {
    ...
    if (status === 'failed') return Promise.reject(shutdownError())
    if (status === 'stopping') {
      return Promise.reject(artiError('ERR_ARTI_CANCELLED', 'Arti addon is stopping'))
    }
    ...
    starting = new Promise((resolve, reject) => {
      ...
      Promise.resolve(nativeStarting).then(
        ...
        (error) => {
          ...
          const mapped = nativeError(error)
...
function validateAddonOptions(options, dependencies) {
  ...
  try {
    ...
    if (
      ((platform === 'android' || platform === 'ios') && finalStat.isSymbolicLink()) ||
