...
function childEnvironment(environment, options) {
  ...
  if (options.reachableAddresses !== undefined) {
    result.BARE_ARTI_REACHABLE_ADDRESSES = options.reachableAddresses.join(',')
  } else {
    delete result.BARE_ARTI_REACHABLE_ADDRESSES
  }
...
function createSidecar(dependencies) {
  ...
  return function startSidecar(options = {}) {
    ...
    let spawnConfirmed = child.pid !== undefined && child.pid !== null
    ...
    function onTerminated(code) {
      ...
      child.removeListener('spawn', onSpawned)
      resolveExit()
    ...
    function onChildError(error) {
      if (status === 'starting' && !spawnConfirmed) {
        waitForUnspawned(bootstrapError('Could not spawn arti-socks', error)).catch(() => {})
        return
      }
      if (status === 'starting') {
    ...
    function waitForUnspawned(error) {
      ...
      createStopPromise()
      waitForExit()
      armUnspawnedWatchdog(error)
      return stopPromise
    }

    function armUnspawnedWatchdog(error) {
      try {
        shutdownTimer = setShutdownTimer(() => {
          shutdownTimer = null
          if (exitConfirmed || status === 'failed') return
          failShutdown(shutdownError('Could not confirm that arti-socks never spawned', error))
        }, shutdownTimeout)
      } catch (timerError) {
        failShutdown(shutdownError('Could not monitor unspawned arti-socks', timerError))
      }
    }
