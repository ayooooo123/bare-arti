...
function createBackend({ platform, arch, loadAddon, startSidecar }) {
  ...
  function startAddon(options) {
    if (owner) {
      ...
      return addon.start({ ...options, backend: 'addon' })
    }
    ...
    let starting
    try {
      starting = selected.start({ ...options, backend: 'addon' })
    } catch (error) {
      starting = Promise.reject(error)
    }
    startingOwner.starting = starting
    Promise.resolve(starting).then(null, () => {
