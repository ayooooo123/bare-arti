...
const { matchingReachableAddresses } = require('./reachable-addresses')
...
function matchingSidecarConfig(left, right) {
  return (
    ...
    matchingReachableAddresses(left.reachableAddresses, right.reachableAddresses) &&
...
function createBackend({ platform, arch, loadAddon, startSidecar }) {
  ...
  function getAddon() {
    ...
    try {
      ...
      if (error && error.code === 'ERR_ARTI_ADDON_INCOMPATIBLE') throw error
    ...
    if (
      ...
      typeof candidate.matchesOptions !== 'function'
    ) {
      throw missingAddon(new Error('Addon does not expose start(), stop(), and matchesOptions()'))
    }
    addon = candidate
    return addon
  }
  ...
  function startAddon(options) {
    if (owner) {
      ...
      let matches
      try {
        matches = owner.matchesOptions(options)
      } catch (error) {
        return Promise.reject(error)
      }
      if (!matches) return Promise.reject(conflict())
    ...
    owner = {
      kind: 'addon',
      status: 'active',
      matchesOptions: (nextOptions) => selected.matchesOptions(nextOptions),
      stop: () => selected.stop(),
      stopping: null,
      unsubscribe: null
    }
    ...
    let nativeStarting
    try {
      nativeStarting = selected.start(options)
    } catch (error) {
  ...
  function startSelectedSidecar(options) {
    ...
    let resolveStarting
    let rejectStarting
    const sharedStarting = new Promise((resolve, reject) => {
    ...
    try {
      ...
      if (owner === startingOwner) owner = null
    ...
    if (
      ...
      if (owner === startingOwner) owner = null
      rejectStarting(artiError('ERR_ARTI_BOOTSTRAP', 'Invalid sidecar operation'))
      return sharedStarting
    }

    startingOwner.status = 'active'
    startingOwner.stop = operation.stop
    Promise.resolve(operation.promise).then(resolveStarting, rejectStarting)
  ...
  function stop() {
    ...
    if (owner.status === 'entering') return Promise.reject(cancelled())
